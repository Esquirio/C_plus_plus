...
int solution(vector<string> &E) {
  ...
  // Vector used to store the table with the list of the employees for each day,
  // after removing the employees that already went  in the first day.
  vector<vector<int>> secondDays = {vector<int>(), vector<int>(), vector<int>(),
                                    vector<int>(), vector<int>(), vector<int>(),
                                    vector<int>(), vector<int>(), vector<int>(),
                                    vector<int>()};

  // Matrix with the pair with the sum of the number of the employees for each
  // pair of days
  vector<vector<int>> pairs = {vector<int>(), vector<int>(), vector<int>(),
                               vector<int>(), vector<int>(), vector<int>(),
                               vector<int>(), vector<int>(), vector<int>(),
                               vector<int>()};

  // Auxiliary string vector with the keys and values after removing the
  // employees that already went in the first day
  ...
  for (vector<int> day : days) {
    ...
    for (vector<string>::iterator it = E_aux.begin(); it != E_aux.end(); it++) {
      ...
      for (int employee : day) {
        ...
        if (it - E_aux.begin() == employee) {
          *it = "";  // Replace the days string by an empty string, meaning that
                     // the employee will not go to another day.
        }
      }
    }

    secondDays.clear();
    secondDays = {vector<int>(), vector<int>(), vector<int>(), vector<int>(),
                  vector<int>(), vector<int>(), vector<int>(), vector<int>(),
                  vector<int>(), vector<int>()};

    // Create the list of employees for the second day without the employee that
    // went to the first day
    int j = 0;
    ...
    for (vector<vector<int>>::iterator it = secondDays.begin();
         it != secondDays.end(); it++) {
      pairs.at(i).insert(pairs.at(i).begin() + j,
                         day.size() + secondDays.at(j).size());
      j++;
    }
