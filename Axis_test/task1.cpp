...
string solution(vector<int> &T) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    int max = *(vEnd - 1),
        min = *vStart;  // Get the max and min value in the current section
    ampl[i] = abs(
        max - min);  // Calculate and store the amplitude of the current section

    // Compare and retain the highest amplitude and the id of the section with
