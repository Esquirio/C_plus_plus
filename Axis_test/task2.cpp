...
int solution(string &S) {
  ...
  // int var;
  // Find the first occurance of the current letter starting from the current
  // position Iterate/loops while not reaching the end of the string
  while ((S.find(currentLetter, currentPosition)) != string::npos) {
    auto var = S.find(currentLetter,
                      currentPosition);  // Store the position of the first
                                         // occurance of teh current letter
    ...
    if (var == currentPosition) {
      ...
      var = S.find(currentLetter,
                   currentPosition);  // Find the position of the new letter
    }

    countBlocks[('b' - currentLetter)]++;

    cout << "index: " << ((currentLetter));
    cout << " a : " << countBlocks[0] << " b :" << countBlocks[1] << endl
         << endl;
    ...
    if (var != string::npos) {
      ...
      blocks.push_back(S.size() -
                       currentPosition);  // Store the size of the final block
    }

    countLetters[('b' - currentLetter)] += blocks.back();
    cout << "Times a : " << countLetters[0] << " Times b :" << countLetters[1]
         << endl
         << endl;
    // Get the largest one
    maxLenght < blocks.back() ? maxLenght = blocks.back()
                              : maxLenght;  // Compute the max block length
  }

  for (int value : blocks) {
    // Sum of largest size - all
    result += maxLenght - value;  // Calcute the difference between the longest
                                  // block with all blocks and sum everything
  }
